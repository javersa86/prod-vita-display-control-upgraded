...
Rectangle {
    ...
    {
        passfailText.visible = true
        passfailText.text = "Calibration Success (" + zero_manager.zeroPIP.toFixed(1) + ") (" + zero_manager.verifiedZeroPIP.toFixed(1) + ")"
        passfailText.color = Style.affirmative
        passfail.visible = true
        passfailIcon.source = "../../iconography/thumbs-up.svg"
        colorOverlay.color = Style.affirmative
...
//            }
            ...
            Rectangle
            {
                ...
                Text {
                    id: actualZeroValue
                    text: zeroState ? state_manager.service_notification_vector[2].toFixed(1) : state_manager.zeroPIP.toFixed(1)
                    color: Style.primary_light
            ...
            Rectangle
            {
                ...
                Text {
                    id: lastZeroValue
                    text: zero_manager.zeroPIP.toFixed(1)
                    color: Style.primary_light
                    font: Style.settingPageTitle
                    anchors.centerIn: parent
                }
            ...
            VerifyButton
            {
                ...
                onClicked:
                {
                    tmpValue = state_manager.service_notification_vector[2]
                    if ( 0 < tmpValue && tmpValue < 5)
                    {
                        state_manager.setZeroPIP(tmpValue)
                        setZeroPass()
                        zeroState = false
                        verifyState = true
                    }
...
//            }
            ...
            Rectangle
            {
                ...
                Text {
                    id: lastVerifyValue
                    text: zero_manager.verifiedZeroPIP.toFixed(1)
                    color: Style.primary_light
                    font: Style.settingPageTitle
                    anchors.centerIn: parent
                }
    ...
    property string pipText: "If the PIP Sensor is reading measurements while ventilation is inactive, the service technician " +
                             "will be required to conduct a thorough calibration procedure.<br><br>" +

                             "To begin calibration, we need zero the PIP Sensor by ensuring that the actual value, averaged data, " +
                             ...
                             "NOTE: Verify that the Proximal tube is connected to the front of the NVENT VITA and that it is open to the atmosphere.<br><br>" +

                             "To verify the Peak Inspiratory Pressure Measurement is accurate, the following steps need to be conducted:<br><br>" +
                             "1. Connect the Proximal tube to an external pressure measuring device. <br>" +
                             "2. Apply 100 cmH<sub>2</sub>O to the Proximal tube. <br>" +
