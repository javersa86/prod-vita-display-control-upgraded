//Modules for api.

/**
 * @ingroup communicationModule
 * @defgroup apiModule Communication API Controller
 * @brief Documentation for the main API Controller.
 */

/**
 * @ingroup  communicationModule
 * @defgroup apiConstsModule Communication API Constants
 * @brief Doucmentation for all API Contant values.
 */

/**
 * @ingroup  communicationModule
 * @defgroup devicesModule Communication Devices
 * @brief Documentation for tracking mounted devices to the display controller.
 */

/**
 * @ingroup  communicationModule
 * @defgroup messageModule Communication Message
 * @brief Documentation for Message component utilized for sending bytes to serial buffer.
 */

/**
 * @ingroup communicationModule
 * @defgroup messageQueueModule Communication Message Queue
 * @brief Documentation for all messages that will be queued up to the serial buffer.
 */

/**
 * @ingroup  communicationModule
 * @defgroup serialModule Communication Serial Port
 * @brief Documentation for Serial Port for the display and system controllers.
 */
